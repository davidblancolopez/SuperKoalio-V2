...
public class Superkoalio extends ApplicationAdapter {
	...
	private void updateKoala (float deltaTime) {
		...
		if(!mapa2){
			...
			if (koala.position.y <= 0 || koala.position.x >= 100 && koala.position.x <= 128 && koala.position.y == 1 )
			{
		...
		if(!mapa2){
			//CONDICION PARA CUANDO SE LLEGA AL FINAL EL KOALA VUELVA AL PRINCIPIO.
			if (koala.position.x >= 315) {
				music.stop();
			/*Gdx.app.log("Koala", "Has ganado");
			create();
			Gdx.app.log("Posicion Y", koala.position.y + "");*/
				cargarMapa();
				//AÑADIDO CAMBIO (LAS VIDAS Y EL CHECKPOINT SE RESETEA)
				vidas = 3;
				checkPoint = false;
			}
		}else{
			//CONDICION PARA CUANDO SE LLEGA AL FINAL EL KOALA VUELVA AL PRINCIPIO.
			if (koala.position.x >= 167) {
				music.stop();
			/*Gdx.app.log("Koala", "Has ganado");
			create();
			Gdx.app.log("Posicion Y", koala.position.y + "");*/
		...
		//CONDICION PARA LOS CHECKPOINT.
		if (!checkPoint && koala.position.x >= 236){
			checkPoint = true;
		}
		...
		//TELETRANSPORTE
		if (koala.position.x >= 11 && koala.position.x <= 12 && koala.position.y == 10){
			koala.position.set(239, 8);
			/*camera.position.x = 239;
			camera.position.y = 8;*/
		}
	...
	/**
	 * Metodo para cargar el siguiente escenario, en este escenario si se acaba la partida.
	 */
